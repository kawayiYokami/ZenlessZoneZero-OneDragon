...
**没有 `press` 字段。** 按键时长不是通过一个独立字段控制的，而是由以下两类机制组合而成：

- **点击类动作**（`按键-普通攻击` / `按键-特殊攻击` 等不带 `-按下` / `-松开` 后缀）：单次执行 = 按一下并立即松开。动作总时长靠 `repeat`（重复次数）+ `post_delay`（每次之后的等待秒数）堆出来。
- **长按类动作**：必须显式拆成 `按键-X-按下` → `等待秒数` → `按键-X-松开` 三步，禁止用任何字段把「按下并保持 N 秒」一行写完。

动作时长的参考值（用「连段时长 ≈ repeat × (动画间隔 + post_delay)」估算）：

| 动作类型 | 总时长 | 典型写法 |
|---|---|---|
| 普攻一套 | 约 5 秒 | `按键-普通攻击` + `repeat` + `post_delay`，参考雅的写法 |
| 强化特殊技 | 约 2 秒 | 单次 `按键-特殊攻击` + 视情况 `post_delay` 留动画 |
| 终结技 | 因角色而异，必须按实测 CG + 无敌总时长配置（参见 4.5） | 参考雨果、星辉比利、佩洛伊斯的模板 |
| 连携攻击 / 支援攻击 | 约 3 秒（2 动作 + 1 动画） | 单次 `按键-连携技-左/右` 或 `按键-快速支援` + `post_delay: 1` |
| 闪避反击 | 约 1 秒 | 参照安比的写法 |
...
`repeat × (动画 + post_delay)` 凑出预期的连段时长；用 handler 的 `interrupt_states` 在外层打断，不要在操作模板里塞终止条件。

### 4.5 终结技 / CG / 无敌期范式

终结技（含部分连携、强化技）通常包含两段时长：

- **CG 动画**：游戏接管，玩家操作无效，但战斗内的失衡计时仍在走。
- **无敌动作**：可输入，全程无敌，需要持续按键吃完动作。

每个终结技都要按角色实测，**总时长 = CG + 无敌**，必须查角色资料确认两段秒数后再写模板。配置范式：

```yaml
operations:
  # 1. 抵消 CG 期间被错算的失衡时间（CG 几秒就 -几）
  - op_name: "设置状态"
    state: "自定义-失衡时间"
    seconds_add: -3        # 例：CG 3 秒
  # 2. 用动作不打断覆盖整段无敌期，避免被其他 handler 打断
  - op_name: "设置状态"
    state: "自定义-动作不打断"
    seconds: 7             # 例：总时长 7 秒
  # 3. 按键阶段：repeat × post_delay 凑动作秒数；多段动作就分多个按键块
  - op_name: "按键-终结技"
    post_delay: 0.1
    repeat: 30             # 3 秒
  # 4. 收尾用「等待秒数」吃完剩余无敌帧，不要用空按键
  - op_name: "等待秒数"
    seconds: 4
```
...
- **`自定义-失衡时间 seconds_add` 必须等于 CG 秒数的负值**。CG/动画期间游戏内的失衡计时会暂停，但脚本上层按真实秒数推进 `自定义-失衡时间`，相当于白白扣掉 CG 那几秒；用 `seconds_add: -<CG秒数>` 把这部分补回来，避免提前判失衡结束。参考 [雨果-终结技.sample.yml](../config/auto_battle_operation/雨果-终结技.sample.yml) 的 `-1`，对应雨果 1s CG。
- **`自定义-动作不打断` 的 seconds 必须等于「CG + 无敌」总时长**，不能少。
- **不要在按键块末尾「清除状态」`自定义-动作不打断`**：动作不打断会自动随时间过期，提前清除会让 handler 在动画收尾时切人。
- **多段输入**：像佩洛伊斯需要「按终结技 3s + 按方向 3s」时，拆成两个按键块顺序排列，方向键用 `按键-移动-前/后/左/右`。
- **收尾留 `等待秒数`**：让剩余无敌帧/动画自然走完后再交给下一个 handler。

### 长按操作的写法

部分角色（本、普罗米娅、佩洛伊斯等）需要长按蓄力。**必须**拆成「按下 → 等待 → 松开」三步：
...
如果蓄力结束的时机由游戏状态决定，则把「等待 + 松开」放进 handler 的 `interrupt_states` 控制路径里，由外层决策来触发松开。

### 通用建议

- 每个操作后的 `post_delay` 保持 `0.1` ~ `0.2` 秒，满足引擎 0.1 秒最小按键间隔。
- 复杂连招应拆分为多个操作模板后组装（参见 4.3）。
- 参考已有角色模板：雅、安比、艾莲等。
...
#----------------------------------------------------------------------
# 7. 参考脚本示例
# 以下两个角色的完整脚本配置，可作为编写新角色时的参考范本。
#----------------------------------------------------------------------

## 7.1 星见雅（最复杂示例）

星见雅的脚本涵盖了目前最完整的一套机制，适合作为**复杂角色**的参考。

**路径：**

- 状态模板：`config/auto_battle_state_handler/站场模板-雅.yml`、`速切模板-雅.yml`
- 操作模板：`config/auto_battle_operation/雅-*.yml`（含拔刀斩、特殊攻击二连、闪A、终结技等）
...
普罗米娅的脚本结构清晰、覆盖了大部分角色的通用模式，适合作为**标准角色**的参考模板。

**路径：**

- 状态模板：`config/auto_battle_state_handler/速切模板-普罗米娅.yml`
- 操作模板：`config/auto_battle_operation/普罗米娅-*.yml`（含普通攻击、特殊攻击、闪A、终结技、连携攻击、长按普攻等）

**包含的典型机制：**

- 标准站场输出循环（普攻→特殊技→闪避反击）
- 连携技与终结技的触发逻辑
- 长按普攻的按下/松开模式
- 体现实战中最常见的“条件判断→执行连招→设置状态”模式

> **建议**：新增角色时，从 `agent_definition.md` 完成角色定义后，可以先对照普罗米娅的脚本结构创建基础文件，再参照星见雅补充复杂机制。
