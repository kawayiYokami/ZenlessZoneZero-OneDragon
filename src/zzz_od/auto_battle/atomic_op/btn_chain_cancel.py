...
class AtomicBtnChainCancel(AtomicOp):

    def __init__(self, ctx: AutoBattleContext, press: bool = False, press_time: Optional[float] = None,
                 release: bool = False):
        if press:
            op_name = BattleStateEnum.BTN_CHAIN_CANCEL.value + '按下'
        elif release:
            op_name = BattleStateEnum.BTN_CHAIN_CANCEL.value + '松开'
        else:
            op_name = BattleStateEnum.BTN_CHAIN_CANCEL.value
    ...
    def execute(self):
        ...
        if self.press and self.press_time is not None and self.press_time > 0:
            ...
            while time.time() - start_time < self.press_time:
                self.ctx.chain_cancel(press=True)
                if self._stop_event.wait(0.02):
                    return

            if not self._stop_event.is_set():
                self.ctx.chain_cancel(release=True)
        else:
            self.ctx.chain_cancel(press=self.press, release=self.release)
