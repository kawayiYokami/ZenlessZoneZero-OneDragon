...
class AtomicBtnNormalAttack(AtomicOp):
    ...
    def __init__(self, ctx: AutoBattleContext, press: bool = False, press_time: Optional[float] = None,
                 release: bool = False):
        if press:
            op_name = BattleStateEnum.BTN_SWITCH_NORMAL_ATTACK.value + '按下'
        elif release:
            op_name = BattleStateEnum.BTN_SWITCH_NORMAL_ATTACK.value + '松开'
        else:
            op_name = BattleStateEnum.BTN_SWITCH_NORMAL_ATTACK.value
    ...
    def execute(self):
        ...
        if self.press and self.press_time is not None and self.press_time > 0:
            ...
            while time.time() - start_time < self.press_time:
                self.ctx.normal_attack(press=True)
                if self._stop_event.wait(0.02):
                    return

            if not self._stop_event.is_set():
                self.ctx.normal_attack(release=True)
        else:
            self.ctx.normal_attack(press=self.press, release=self.release)
