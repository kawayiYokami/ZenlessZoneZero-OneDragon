...
def check_cnt_by_color_range(
    ...
    # 如果状态名包含终结技可用，则输出mask的面积
    if '终结技可用' in state_def.state_name:
        # 输出此时状态名和面积
        log.info('状态名: %s', state_def.state_name)
        log.info('mask面积: %d', cv2.countNonZero(mask))
    ...
    # 输出连通要求和连通的数量
    if '终结技可用' in state_def.state_name:
        log.info('连通要求: %d', state_def.connect_cnt)
        log.info('连通数量: %d', count)
