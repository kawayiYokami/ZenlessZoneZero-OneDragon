...
class LostVoidChooseCommon(ZOperation):
    ...
    def click_confirm(self) -> OperationRoundResult:
        ...
        # 点击返回按钮，处理弹窗 fallback
        result = self.round_by_find_and_click_area(screen_name='迷失之地-通用选择', area_name='按钮-返回',
                                                 success_wait=1, retry_wait=1)
        if result.is_success:
            return self.round_success(result.status)

        # 如果找不到返回按钮，说明有弹窗，先点不再提示，再点确认
        result = self.round_by_find_and_click_area(screen_name='迷失之地-通用选择', area_name='不再提示',
        ...
        # 点确认
        self.round_by_find_and_click_area(screen_name='迷失之地-通用选择', area_name='确认',
                                         success_wait=0.5, retry_wait=0.5)
